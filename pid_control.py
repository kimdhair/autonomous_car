...
class PIDController:
    def __init__(self, kp, ki, kd, integral_limit, derivative_filter, log=False):
        self.kp = kp
        self.ki = ki
        self.kd = kd
    ...
    def update(self, error, dt):
        ...
        # 적분값이 일정 범위를 초과하지 않도록 제한
        self.integral = max(min(self.integral, self.integral_limit), -self.integral_limit)

        derivative = (error - self.previous_error) / dt

        # 로우 패스 필터 적용
        derivative_filtered = self.derivative_filter * derivative + (1 - self.derivative_filter) * self.previous_derivative
        ...
        if self.log:
            print(f"Error: {error}, Integral: {self.integral}, Derivative: {derivative_filtered}, Output: {output}")
...
# PID 제어기 초기화
pid_controller_left = PIDController(kp=2.4, ki=0.05, kd=0.1, integral_limit=10, derivative_filter=0.15, log=True)  # 좌측 모터 제어기 
pid_controller_right = PIDController(kp=3, ki=0.075, kd=0.1, integral_limit=10, derivative_filter=0.15, log=True)  # 우측 모터 제어기
